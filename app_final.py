...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .auth-card h2 { text-align: center; color: #2E7D32; margin-bottom: 24px; }
        ...
        input, select, textarea { width: 100%; padding: 12px; margin-bottom: 10px; border: 1px solid #ddd; border-radius: 10px; font-size: 14px; }
...
<div class="container" id="app">
    ...
    <div id="app-screen" style="display:none;">
        <div class="app-header">
            <div class="app-title"><h1>📋 AutoAgenda</h1><p>"Configura una vez. Agenda siempre"</p></div>
            <div class="user-info"><span class="user-name" id="userNombre"></span><button class="logout-btn" onclick="logout()">Salir</button></div>
        </div>
        <div class="tabs">
        ...
        <div id="tab-servicios" class="tab-content active">
            <div class="card"><h2>➕ Agregar Servicio</h2><input id="nuevoServicio" placeholder="Nombre del servicio"><button onclick="agregarServicio()">Agregar</button></div>
            <div class="card"><h2>📋 Mis Servicios</h2><div id="listaServicios"></div></div>
        </div>
        <div id="tab-horarios" class="tab-content">
            <div class="card"><h2>⏰ Horarios Laborales</h2><div id="listaHorarios"></div><button onclick="guardarHorarios()">Guardar Horarios</button></div>
        </div>
        <div id="tab-bloqueos" class="tab-content">
            ...
            <label style="display:flex; align-items:center; gap:8px; margin:10px 0;"><input type="checkbox" id="bloqueoTodoDia" checked onchange="toggleBloqueoHorario()"> Día completo</label>
            ...
            <input id="bloqueoMotivo" placeholder="Motivo (opcional)"><button onclick="agregarBloqueo()">Bloquear Rango</button></div>
        ...
        <div id="tab-agendar" class="tab-content">
            ...
            <input type="time" id="citaHora" step="1800" placeholder="Hora"><input id="citaNombre" placeholder="Nombre del cliente *"><input id="citaTelefono" placeholder="Teléfono">
            <select id="citaServicio"></select><textarea id="citaMotivo" rows="2" placeholder="Motivo / Observación"></textarea><button onclick="agendarCita()">Agendar Cita</button></div>
        </div>
        <div id="tab-citas" class="tab-content">
            <div class="card"><h2>📅 Citas por Fecha</h2><input type="date" id="filtroFecha" onchange="cargarCitas()"><div class="tabla-container" id="listaCitas"></div></div>
        </div>
        <div id="tab-resumen" class="tab-content">
            <div class="card"><h2>📊 Resumen del Día</h2><div class="numero-grande" id="totalHoy">0</div><p style="text-align:center;">Citas para hoy</p></div>
            <div class="card"><h2>📋 Todas las Citas</h2><div class="tabla-container" id="todasCitas"></div></div>
        </div>
    </div>
</div>
<script>
    ...
    async function login() {
        ...
        if(res.ok){
            ...
            await cargarApp();
        }else mostrarMensaje('Email o contraseña incorrectos','error');
    }
    async function registro() {
        ...
        if(res.ok){mostrarMensaje('Registro exitoso. Inicia sesión','exito');mostrarLogin();}
        else mostrarMensaje('Email ya registrado','error');
    }
    async function logout() {
        if(token) await fetch('/api/logout',{method:'POST',headers:{'Authorization':`Bearer ${token}`}});
        localStorage.removeItem('token');localStorage.removeItem('userNombre');token=null;
        document.getElementById('app-screen').style.display='none';document.getElementById('login-screen').style.display='block';
    }
    function mostrarRegistro(){document.getElementById('login-form').style.display='none';document.getElementById('registro-form').style.display='block';}
    function mostrarLogin(){document.getElementById('login-form').style.display='block';document.getElementById('registro-form').style.display='none';}
    async function cargarApp(){
        ...
        document.getElementById('citaFecha').value = hoy;document.getElementById('filtroFecha').value = hoy;
        await cargarServicios();await cargarHorarios();await cargarBloqueos();await cargarSelectServicios();await cargarHorariosDisponibles();
    }
    async function fetchApi(url, options={}){const headers={'Authorization':`Bearer ${token}`,'Content-Type':'application/json'};return fetch(url,{...options,headers});}
    ...
    async function cargarServicios(){const res=await fetchApi('/api/servicios');const s=await res.json();document.getElementById('listaServicios').innerHTML=s.map(s=>`<div class="servicio-item"><span>📌 ${s.nombre}</span><button class="danger" onclick="eliminarServicio(${s.id})">Eliminar</button></div>`).join('');}
    async function cargarSelectServicios(){const res=await fetchApi('/api/servicios');const s=await res.json();document.getElementById('citaServicio').innerHTML=s.map(s=>`<option value="${s.nombre}">${s.nombre}</option>`).join('');}
    async function agregarServicio(){const nombre=document.getElementById('nuevoServicio').value.trim();if(!nombre){mostrarMensaje('Escribe un nombre','error');return;}const res=await fetchApi('/api/servicios',{method:'POST',body:JSON.stringify({nombre})});if(res.ok){document.getElementById('nuevoServicio').value='';cargarServicios();cargarSelectServicios();mostrarMensaje('Servicio agregado','exito');}else mostrarMensaje('Error o ya existe','error');}
    async function eliminarServicio(id){if(confirm('¿Eliminar este servicio?')){await fetchApi(`/api/servicios/${id}`,{method:'DELETE'});cargarServicios();cargarSelectServicios();mostrarMensaje('Servicio eliminado','exito');}}
    async function cargarHorarios(){const res=await fetchApi('/api/horarios');const h=await res.json();const dias=['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];let html='';for(let i=0;i<h.length;i++){html+=`<div class="horario-row"><span class="horario-dia">${dias[i]}</span><label class="horario-check"><input type="checkbox" id="activo_${i}" ${h[i].activo?'checked':''}> Activo</label><input type="time" id="inicio_${i}" value="${h[i].inicio}" class="horario-time" ${!h[i].activo?'disabled':''}><span>-</span><input type="time" id="fin_${i}" value="${h[i].fin}" class="horario-time" ${!h[i].activo?'disabled':''}></div>`;}document.getElementById('listaHorarios').innerHTML=html;for(let i=0;i<7;i++){const cb=document.getElementById(`activo_${i}`);if(cb) cb.onchange=()=>{document.getElementById(`inicio_${i}`).disabled=!cb.checked;document.getElementById(`fin_${i}`).disabled=!cb.checked;}}}
    async function guardarHorarios(){const data=[];for(let i=0;i<7;i++)data.push({dia:i,activo:document.getElementById(`activo_${i}`).checked,inicio:document.getElementById(`inicio_${i}`).value,fin:document.getElementById(`fin_${i}`).value});await fetchApi('/api/horarios',{method:'POST',body:JSON.stringify(data)});mostrarMensaje('Horarios guardados','exito');}
    ...
    async function agregarBloqueo(){const fi=document.getElementById('bloqueoFechaInicio').value,ff=document.getElementById('bloqueoFechaFin').value,td=document.getElementById('bloqueoTodoDia').checked,i=document.getElementById('blockInicio')?.value,f=document.getElementById('blockFin')?.value,m=document.getElementById('bloqueoMotivo').value;if(!fi||!ff){mostrarMensaje('Selecciona fechas','error');return;}const res=await fetchApi('/api/bloqueos',{method:'POST',body:JSON.stringify({fecha_inicio:fi,fecha_fin:ff,todo_dia:td,inicio:i,fin:f,motivo:m})});if(res.ok){document.getElementById('bloqueoFechaInicio').value='';document.getElementById('bloqueoFechaFin').value='';document.getElementById('bloqueoMotivo').value='';cargarBloqueos();mostrarMensaje('Rango bloqueado','exito');}else mostrarMensaje('Error','error');}
    async function cargarBloqueos(){const res=await fetchApi('/api/bloqueos');const b=await res.json();if(b.length===0)document.getElementById('listaBloqueos').innerHTML='<p style="color:#999;">No hay días bloqueados</p>';else document.getElementById('listaBloqueos').innerHTML=b.map(b=>`<div class="servicio-item"><span>🚫 ${b.fecha_inicio} a ${b.fecha_fin} - ${b.todo_dia?'Día completo':b.inicio+' a '+b.fin} ${b.motivo||''}</span><button class="danger" onclick="eliminarBloqueo(${b.id})">Eliminar</button></div>`).join('');}
    ...
    async function cargarHorariosDisponibles(){const fecha=document.getElementById('citaFecha').value;if(!fecha)return;const res=await fetchApi(`/api/horarios-disponibles/${fecha}`);const data=await res.json();const c=document.getElementById('horariosDisponiblesDiv');if(data.horarios.length===0)c.innerHTML='<div style="color:#C62828;">No hay horarios disponibles</div>';else c.innerHTML=`<div class="horarios-grid">${data.horarios.map(h=>`<div class="horario-sugerido" onclick="document.getElementById('citaHora').value='${h}'">${h}</div>`).join('')}</div>`;}
    async function cargarCitas(){const fecha=document.getElementById('filtroFecha').value||new Date().toISOString().split('T')[0];document.getElementById('filtroFecha').value=fecha;const res=await fetchApi(`/api/citas?fecha=${fecha}`);const c=await res.json();const cont=document.getElementById('listaCitas');if(c.length===0)cont.innerHTML='<p style="color:#999;">No hay citas para esta fecha</p>';else cont.innerHTML=`<table><thead><tr><th>Hora</th><th>Cliente</th><th>Servicio</th><th></th></tr></thead><tbody>${c.map(c=>`<tr><td>${c.hora}</td><td>${c.nombre}</td><td>${c.servicio}</td><td><button class="danger" onclick="eliminarCita(${c.id})">X</button></td></tr>`).join('')}</tbody></table>`;}
    async function eliminarCita(id){if(confirm('¿Cancelar esta cita?')){await fetchApi(`/api/citas/${id}`,{method:'DELETE'});cargarCitas();cargarResumen();cargarHorariosDisponibles();mostrarMensaje('Cita eliminada','exito');}}
    async function agendarCita(){const fecha=document.getElementById('citaFecha').value,hora=document.getElementById('citaHora').value,nombre=document.getElementById('citaNombre').value.trim(),telefono=document.getElementById('citaTelefono').value,servicio=document.getElementById('citaServicio').value,motivo=document.getElementById('citaMotivo').value;if(!fecha){mostrarMensaje('Selecciona fecha','error');return;}if(!hora){mostrarMensaje('Selecciona hora','error');return;}if(!nombre){mostrarMensaje('Escribe nombre','error');return;}const res=await fetchApi('/api/citas',{method:'POST',body:JSON.stringify({fecha,hora,nombre,telefono,servicio,motivo})});if(res.ok){document.getElementById('citaNombre').value='';document.getElementById('citaTelefono').value='';document.getElementById('citaMotivo').value='';document.getElementById('citaHora').value='';cargarCitas();cargarResumen();cargarHorariosDisponibles();mostrarMensaje('Cita agendada','exito');}else mostrarMensaje('Horario no disponible','error');}
    async function cargarResumen(){const hoy=new Date().toISOString().split('T')[0];const res=await fetchApi(`/api/citas?fecha=${hoy}`);const citasHoy=await res.json();document.getElementById('totalHoy').innerText=citasHoy.length;const resTodas=await fetchApi('/api/citas');const todas=await resTodas.json();const cont=document.getElementById('todasCitas');if(todas.length===0)cont.innerHTML='<p>No hay citas agendadas</p>';else cont.innerHTML=`<table><thead><tr><th>Fecha</th><th>Hora</th><th>Cliente</th><th>Servicio</th></tr></thead><tbody>${todas.map(c=>`<tr><td>${c.fecha}</td><td>${c.hora}</td><td>${c.nombre}</td><td>${c.servicio}</td></tr>`).join('')}</tbody></table>`;}
    if(token) cargarApp();
</script>
</body>
</html>'''
